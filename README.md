...
│   │   ├── modules               # js-модули (можно подключить для отдельных страниц)
...
Благодаря плагину __gulp-pug__ вы можете разделять файлы на различные шаблоны, которые должны храниться в папке __partials__. Удобно делить страницу на секции.

> Для вставки html-частей в главный файл используйте `include partials/about`

Если вы хотите создать многостраничный сайт - копируйте __index.pug__, переименовывайте как вам нужно, и используйте.

При использовании команды `npm run build`, вы получите минифицированный html-код в одну строку для всех файлов.

## Работа с CSS

В сборке используется препроцессор __sass__ в синтаксисе __scss__.

Стили, написанные в __components__, следует подключать в файлы, созданные внутри папки __modules__.

__ВАЖНО:__ Для использования миксинов, импортируйте их в файл, созданный внутри папки __modules:__ `@import '../mixins';`
...
В итоговой папке __app/css__ создаются: 
<br> файлы, созданные вами в папке __modules.__ <br> __main.css__ - для общих стилей, 
<br> __vendor.css__ - для стилей всех библиотек, использующихся в проекте.
...
JS-код лучше делить на компоненты - небольшие js-файлы, которые содержат свою, изолированную друг от друга реализацию. Такие файлы помещайте в папку __components__, а потом импортируйте в файл, созданный вами в папке __modules.__

В файле __vars.js__ должны храниться базовые переменные проекта, вроде нахождения элементов и т.д.

В файле __main.js__ имеет назначение общего файла для всех страниц.

Подключать библиотеки в сборку можно только с помощью npm. Для этого установите нужный пакет с помощью его команды, создайте в папке __components__ файлик, импортируйте туда библиотеку и работайте с ней. Не забудьте импортировать этот файл в файл `modules/*.js`.
...
В сборке доступна поддержка __webp__ и __avif__ формата. Подключить его вы можете через тег `picture`. Для background можно использовать __webp__, либо использовать `image-set` там, где это возможно.
