...
const watcher = () => {
  ...
  gulp.watch(`${app.paths.srcPartialsFolder}/**/*.pug`, htmlInclude);
  gulp.watch(`${app.paths.base.src}/*.pug`, htmlInclude);
  gulp.watch(`${app.paths.resourcesFolder}/**`, resources);
  gulp.watch(`${app.paths.srcImgFolder}/**/**.svg`, images);
  gulp.watch(`${app.paths.srcImgFolder}/**/**.{jpg,jpeg,svg}`, webpImages);
  gulp.watch(`${app.paths.srcImgFolder}/**/**.{jpg,jpeg,svg}`, avifImages);
  gulp.watch(app.paths.srcSvg, svgSprites);
}

const dev = gulp.series(clean, htmlInclude, scripts, styles, resources, images, webpImages, avifImages, svgSprites, watcher);
const backend = gulp.series(clean, htmlInclude, scriptsBackend, stylesBackend, resources, images, webpImages, avifImages, svgSprites);
const build = gulp.series(clean, htmlInclude, scripts, styles, resources, images, webpImages, avifImages, svgSprites, htmlMinify);
